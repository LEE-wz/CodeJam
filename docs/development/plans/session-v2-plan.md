...
**Status:** Implemented and release-verified through Phase 15 on
`bidding-agent-implementation`. This file preserves the original feasibility
analysis; the shipped contract authority is [overview-sessions.md](../overview-sessions.md)
and request evidence is in [STATUS.md](../STATUS.md).
**Author:** Prepared 2026-08-30 against `main` @ `2344f2c`; reconciled with the
completed release on 2026-09-01.
**Scope:** The nine change requests below, all shipped for the
`shared_session_v1` workflow and Session UI.
**Authority note:** The required contract amendments and mini-RFCs were approved
and implemented in Phases 10–15. Historical future-tense sections below explain
the design path, not outstanding work.
...
**Read model.** `GET /api/coordination-runs/:id` returns every turn, attempt, artifact, and event for the run, unpaginated ([`routes.ts`](../../../apps/server/src/coordination/routes.ts)), and the UI polls it every 1.5s while active (now [`SessionWorkspace.tsx`](../../../apps/web/src/SessionWorkspace.tsx)).
...
Frontend-only removal. Delete the workflow toggle, role selects, required-sections editor, `maxRevisions`, `ArtifactCard`, and the verified branches of `validateForm` in what is now [`SessionWorkspace.tsx`](../../../apps/web/src/SessionWorkspace.tsx) (~8 references), trim [`coordination-types.ts`](../../../apps/web/src/coordination-types.ts), and rewrite the verified fixtures in [`coordination-fixtures.ts`](../../../apps/web/src/testing/coordination-fixtures.ts) and the verified cases in [`SessionWorkspace.test.tsx`](../../../apps/web/src/SessionWorkspace.test.tsx). The server keeps `verified_handoff_v1` intact, so all 474 server tests stay green and old runs still render. Historical verified runs in the run list should be shown read-only rather than hidden, or filtered out — pick one (§7).
