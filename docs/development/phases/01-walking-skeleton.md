...
- [ ] **P1-01** Implement deterministic selectors for the latest committed proposal, review, and final artifact by turn sequence.
- [ ] **P1-02** Route new run → Planner, proposal → Critic, rejection → Planner revision, approval → Finaliser, and final artifact → complete.
- [ ] **P1-03** Enforce `maxRevisions` and `maxTurns`; map impossible durable state to safe `INVALID_STATE` failure.
