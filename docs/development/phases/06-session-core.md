...
- [ ] **P6-04** Implement the strict, bounded Zod schema for `SessionMessagePayload` in `coordination/schemas.ts`: `schemaVersion: 1`, `type: "session_message"`, `content` trimmed and 1..500 characters, `done` optional boolean, free-chat only; object is strict. The countdown cross-field rule constrains countdown turns further.
