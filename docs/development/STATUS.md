...
| P15-12 | `in_progress` — 1 of 4 recorded | **Root cause of the earlier block found and fixed:** it was never a rate limit. `.env` pointed at `deepseek-v4-flash-ga-260731`, which the account has not activated (`ModelNotOpen`); `ARK_MODEL` is now `glm-4-7-251222`, confirmed working end to end through Codex. **A second, larger problem was found:** the running Compose image was built `2026-08-30`, *before* Phase 14 — its bundle had no `sessionPlanning` at all, so the deployed stack was two phases behind `main`. Rebuilt and restarted. Two demo Agents created (`Demo Coordinator`, `Demo Unreliable`). **Recorded:** `33974d52` "P15-12 ordered session", a real 30-turn session with committed `session_plan` artifacts under coordinator planning, three user prompts, and a **verified mid-session restart** (26 turns intact across the restart, then accepted another prompt) — so it covers both the *ordered* and *multi-prompt-survived-restart* fallbacks. Ended, so it is a `completed` run. **Remaining:** the parallel session and the plan-rejection recovery. The parallel attempt (`09b9414e`) failed `MAX_ATTEMPTS_EXCEEDED` on genuine GLM `429`s after the heavy ordered run — a real per-account rate limit that a multi-Agent wave saturates, which is itself the operational point recorded in `COORDINATION_OPERATIONS.md` §6. |
...
| P15-16 | `in_progress` | The **restart** flow is verified live: a session in `awaiting_input` survived a `docker compose restart` with all 26 turns intact and accepted a further prompt. Normal, stop, end, and failure flows remain; `end` was exercised via the API but not through the browser. |
| P15-17 | `complete` | Swept the durable database, an inspected 46 KB live API payload, 811 lines of server log, and the web client. **Database:** no credentials, no authorization headers, no cookies. Lease tokens persist server-side by design (12 occurrences) and are the thing the read model strips. **API payload:** zero `leaseToken`, zero credentials, no auth headers, no stack traces. **Events:** all 39 events carry only allowlisted keys (`agentId`, `promptDigest`, `outputDigest`, `sizeChars`, counts, enums); **zero events carry any content-like key**. A real 114-character transcript snippet and the run objective were both grepped against the full server log: **zero occurrences** — transcript content and objectives never reach the logs. **Web client:** no `console.*` anywhere in `apps/web/src`, and the auth token lives in a module variable only — never `localStorage`, `sessionStorage`, or a cookie. The negative half of the Session v2 clause is additionally pinned by `redaction.test.ts`, which asserts `content`, `prompt`, `objective`, `rawOutput`, `leaseToken`, `authorization`, and `cookie` are all dropped from event details, and that no spelling of a lease token is ever allowlisted. **Positive half now closed** against a live prompted session (`33974d52`): the `user_message` artifact stores the prompt verbatim, carries `createdBy: {kind:"user"}` and a `transcriptSequence`, and the same prompt string appears **zero** times in the server log. Across that run's 30 turns, zero events carried a content-like key. User-authored content therefore appears in artifacts, and only in artifacts. |
