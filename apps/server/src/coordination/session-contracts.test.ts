...
describe("Phase 5 session create", () => {
  it("normalizes and enforces auction policy for direct service callers", async () => {
    ...
    await expect(
      service.createRun({
        ...CREATE_FREE_CHAT_REQUEST,
        policy: {
          ...CREATE_FREE_CHAT_REQUEST.policy,
  ...
  it("initialises a session run with no protocol shared state", async () => {
...
describe("Phase 5 session message payload", () => {
  it("omits done on an ordinary message", () => {
    expect(freeChatPayload("A bounded contribution.")).toEqual({
      schemaVersion: 1,
  ...
  it("routes a valid new session run to the first participant", () => {
    ...
    expect(workflow.decideNext({
      run: {
        ...
        nextTurnSequence: 1,
        activeTurnIds: [],
        lastUserArtifactId: "user-artifact-1",
        version: 1,
        createdAt: FIXED_NOW,
        updatedAt: FIXED_NOW,
      },
      ...
      artifacts: [{
        id: "user-artifact-1",
        runId: "run-session-contract",
        type: "user_message",
        payload: { schemaVersion: 1, type: "user_message", content: "Begin." },
        createdBy: { kind: "user" },
